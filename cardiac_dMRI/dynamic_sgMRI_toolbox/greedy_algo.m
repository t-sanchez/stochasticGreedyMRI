...
% save('im_temp.mat','x_original','k_original')
while rate_i < rate
    % CHANGE IT TO FOR LOOP WHEN DEBUGGING !!!
    ...
    time_init = tic;
        ...
        %computations
        for i = 1:numel(average_objective_value)
        %for loop: 
        ...
        objective_value = zeros(numel(samp),1);
        
        for abc = 1:numel(samp)
            loc = samp(abc);
            
            % Reconstruct data and record performance
            x_recon = reconstruct(k_original(:,:,:,loc), mask_temp, algo);
            objective_value(abc) = compute_error(x_original(:,:,:,loc), x_recon(:,:,:), metric);
